...
* **`preferorigin`**: *(optional)* available as of Endpoint version 3.0, can have a value of `GREEK` or `HEBREW`. If set, will retrieve the verses based on the GREEK original text or the HEBREW original text when there are multiple possibilities. This occurs often in Catholic versions of the Bible for certain Books and chapters (e.g. Esther).
...
* **`info`**: an object containing information not directly associated with the Bible verses returned, but rather with the endpoint itself. Three *key:value* pairs are currently returnd: 
  ...
  * **`detectedNotation`**: this will echo back which notation was used and detected in the request, will have a value of `EUROPEAN` or `ENGLISH`. A value of `MIXED` is bad and would mean that the endpoint engine was not able to determine the notation that was used. There will probably be some error messages associated with this kind of value.
  * **`bibleVersionsInfo`**: an object whose keys are the acronyms of the Bible versions supported. Associated with each key is a string of pipe separated values, when exploded or split these correspond with:
    ...
    4. Whether this version of the Bible has an **Imprimatur** from an Ecclesiastical authority (0=false,1=true)
    5. Whether the canon for this version of the Bible is a CATHOLIC or a PROTESTANT canon (the books of the Bible are different between canons)
    6. Identification of the Copyright holder, if applicable
    7. Notes about this edition of the Bible. These might contain an indication of the source of the text.
...
* **`info`**: an object containing information not directly associated with the Bible verses returned, but rather with the endpoint itself. For now only one *key:value* pair is returned with the current version of the endpoint itself: `"ENDPOINT_VERSION":"3.0"`. This may be useful information because the data produced by the endpoint may change over time, it is useful to know which data is associated with which version of the endpoint. For example, if an application caches data returned by the endpoint, but there has been a change to the structure of the data in a new version of the API, the application would know how to deal with emptying the cache and requesting new data from the updated endpoint.
...
* **`info`**: an object containing a kind of metadata information about the search that was performed.
    ...
    ```javascript
    {"ENDPOINT_VERSION":"3.0","keyword":"light","version":"NABRE"}
    ```
